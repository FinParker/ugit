...
def hash_object(args):
    with open(args.file, 'rb') as f:
        data_content = f.read()
        object_id = data.hash_object(data_content)
        logging.debug(f'file <{args.file}> hashed, SHA1 code:')
        print(object_id)
...
def write_tree(args):
    print('hash all files from current directory:')
    tree_id = base.write_tree()
    print(tree_id)
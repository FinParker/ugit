...
def init():
    ugit_path = os.path.join(os.getcwd(), UGIT_DIR)
    if os.path.exists(UGIT_DIR):
        raise FileExistsError(f'ugit repository already exists in {ugit_path}')
    
    try:
        os.makedirs(UGIT_DIR)
        os.makedirs(os.path.join(UGIT_DIR, 'objects'))
        logging.debug(f'Initialized empty ugit repository in {ugit_path}')
    except OSError as e:
